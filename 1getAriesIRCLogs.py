...
################################################################
# usage:
# > python 1getAriesIRCLogs.py <new_datasource_id> <date-to-start> <password>
#
...
# example usage:
# > python3 1getAriesIRCLogs.py 63307 20150523 password
#
# purpose: 
    ...
    while(dateS <= yesterday):
        ...
        try:
            cursor2.execute(insertQuery,(newDS,
                    forgeID, 
                    'Aries IRC '+ str(yyyy) + str(mm) + str(dd),
                    datetime.datetime.now(),
                    'msquire@elon.edu',
                    fileLoc,
                    datetime.datetime.now(),
